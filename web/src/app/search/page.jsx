...
function DrugCard({ drug }) {
    return (<div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 group">
      ...
      <h3 className="text-xl font-bold mb-2">
        <Link href={`/drugs/${drug.slug}`} className="text-gray-900 hover:text-blue-600 transition-colors group-hover:text-blue-600">
          {drug.drugName}
        </Link>
      </h3>
      
      {drug.label.genericName && (<p className="text-gray-600 mb-2">
          ...
          <div className="max-w-4xl mx-auto">
            <form className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <input type="text" name="q" placeholder="Search by drug name, condition, or active ingredient..." defaultValue={query} className="w-full px-6 py-4 text-gray-900 rounded-full focus:outline-none focus:ring-4 focus:ring-blue-300/50 text-lg"/>
              </div>
              <button type="submit" className="bg-white text-blue-600 px-8 py-4 rounded-full font-semibold hover:bg-blue-50 transition-all shadow-lg hover:shadow-xl whitespace-nowrap">
                Search Drugs
              </button>
            </form>
          </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-lg sticky top-24">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <select name="category" defaultValue={category} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
              ...
              {query ? (<div>
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      {searchResults && (<p className="text-gray-600">
                  ...
                  {searchResults && searchResults.data.length > 0 ? (<>
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {searchResults.data.map((drug) => (<DrugCard key={drug.setId} drug={drug}/>))}
                      </div>
                      
                      {searchResults.pagination.totalPages > 1 && (<div className="flex justify-center mt-12">
                          <div className="flex space-x-2">
                            {Array.from({ length: searchResults.pagination.totalPages }, (_, i) => i + 1).map((pageNum) => (<Link key={pageNum} href={`/search?q=${encodeURIComponent(query)}&page=${pageNum}`} className={`px-4 py-2 rounded-lg font-medium transition-all ${pageNum === page
