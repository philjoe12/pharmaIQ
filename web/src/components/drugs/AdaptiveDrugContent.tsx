...
import { DrugLabel } from '@/lib/api/drugs.api';

interface AdaptiveDrugContentProps {
  drug: DrugLabel;
}

interface EnhancedDrugData {
  enhancedContent?: {
...
export function AdaptiveDrugContent({ drug }: AdaptiveDrugContentProps) {
  const effectiveUserType = useEffectiveUserType();
  const { setUserType } = useUserType();
  const [enhancedData, setEnhancedData] = useState<EnhancedDrugData | null>(null);
  ...
  const renderPatientContent = () => (
    <div className="space-y-8">
      ...
      {enhancedData?.enhancedContent?.keyPoints && (
        <Card className="p-6">
          <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
          ...
          <ul className="space-y-3">
            {enhancedData.enhancedContent.keyPoints.map((point, idx) => (
      ...
      {/* FAQ Section */}
      {enhancedData?.enhancedContent?.faq && enhancedData.enhancedContent.faq.length > 0 && (
        <Card className="p-6">
          <h3 className="text-xl font-semibold mb-4">Frequently Asked Questions</h3>
          <div className="space-y-4">
            {enhancedData.enhancedContent.faq.map((faq, idx) => (
    ...
    <div className="space-y-8">
      ...
      {drug.label.description && (
        <Card className="p-6">
          ...
          <div 
            className="text-gray-700 prose prose-sm max-w-none"
            dangerouslySetInnerHTML={{ __html: drug.label.description }} 
          />
        </Card>
      )}
      ...
      {drug.label.indicationsAndUsage && (
        <Card className="p-6">
          ...
          <div 
            className="text-gray-700 prose prose-sm max-w-none"
            dangerouslySetInnerHTML={{ __html: drug.label.indicationsAndUsage }} 
          />
        </Card>
      )}
      ...
      {drug.label.dosageAndAdministration && (
        <Card className="p-6">
          ...
          <div 
            className="text-gray-700 prose prose-sm max-w-none"
            dangerouslySetInnerHTML={{ __html: drug.label.dosageAndAdministration }} 
          />
        </Card>
      )}
      ...
      {drug.label.contraindications && (
        <Card className="p-6">
          ...
          <div 
            className="text-gray-700 prose prose-sm max-w-none"
            dangerouslySetInnerHTML={{ __html: drug.label.contraindications }} 
          />
        </Card>
      )}
      ...
      {drug.label.warningsAndPrecautions && (
        <Card className="p-6">
          ...
          <div 
            className="text-gray-700 prose prose-sm max-w-none"
            dangerouslySetInnerHTML={{ __html: drug.label.warningsAndPrecautions }} 
          />
        </Card>
      )}
      ...
      {drug.label.adverseReactions && (
        <Card className="p-6">
          ...
          <div 
            className="text-gray-700 prose prose-sm max-w-none"
            dangerouslySetInnerHTML={{ __html: drug.label.adverseReactions }} 
          />
        </Card>
      )}
      ...
      {drug.label.clinicalPharmacology && (
        <Card className="p-6">
          ...
          <div 
            className="text-gray-700 prose prose-sm max-w-none"
            dangerouslySetInnerHTML={{ __html: drug.label.clinicalPharmacology }} 
          />
        </Card>
      )}
    </div>
  ...
  const simplifyMedicalText = (text: string): string => {
    if (effectiveUserType !== 'patient') return text;
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className="text-gray-600">
            {drug.label.genericName && (
              <span>Generic: <strong>{drug.label.genericName}</strong></span>
            )}
            {drug.label.genericName && drug.labeler && <span> • </span>}
            {drug.labeler && (
              <span>Manufacturer: <strong>{drug.labeler}</strong></span>
            )}
          </div>
        </div>
