export function ComparisonTable({ drugs }) {
    if (!drugs.length) {
    ...
    const renderCellValue = (drug, field) => {
      ...
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr>
            ...
            {drugs.map((drug) => (<th key={drug.id} className="border border-gray-300 px-4 py-2 bg-gray-100 text-left">
                {drug.name}
              </th>))}
          </tr>
        </thead>
        <tbody>
          {comparisonFields.map((field) => (<tr key={field.key}>
              <td className="border border-gray-300 px-4 py-2 font-medium bg-gray-50">
                {field.label}
              </td>
              {drugs.map((drug) => (<td key={`${drug.id}-${field.key}`} className="border border-gray-300 px-4 py-2 align-top">
